...
void HTMLShoppingBasket::write() {
    std::fstream html("../ShoppingCart.html", std::ios::out);
    html<<"<!DOCTYPE html>\n"
          "<html>\n"
          "<head>\n"
...
//          "        <td>Photograph Link</td>\n"
    ...
    for (auto const& coat : getVector()) {
        html<<"<tr>\n"
              "<td>"<<coat.getSize()<<"</td>\n"
              "<td>"<<coat.getColor()<<"</td>\n"
//              "<td>"<<coat.getUniqueID()<<"</td>\n"
              "<td>"<<coat.getPrice()<<"</td>\n"
              "<td>"<<coat.getQuantity()<<"</td>\n"
//              "<td><a href=\""<<coat.getPhotographLink()<<"\">Link</a></td>\n"
                                                          "</tr>\n";}
    html<<"</table>\n"
          "</body>\n"
          "</html>";
...
HTMLShoppingBasket::HTMLShoppingBasket(): RepositoryShopping(){}

void CSVShoppingBasket::write() {
    std::fstream csv("../ShoppingCart.csv", std::ios::out);
    if(!csv.is_open()) {
        return;
    }
    for (auto const& i : getVector()) {
        csv<<i.getSize() + "," + i.getColor() + "," + std::to_string(i.getPrice()) + "," + std::to_string(i.getQuantity())<<'\n';
    }
    csv.close();
}
...
CSVShoppingBasket::CSVShoppingBasket(): RepositoryShopping() {}
