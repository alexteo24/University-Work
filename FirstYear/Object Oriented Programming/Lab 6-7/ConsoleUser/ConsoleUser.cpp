...
void ConsoleUser::navigateOffers() {
    ...
    std::string option;
    readCoatSize(size);
    std::vector<Coat> matching = _serviceUser.findMatchingSize(size);
    if (matching.empty()) {
        std::cout<<"Currently, there are no coats matching the required size!\n";
    }
    else {
        bool next = false;
        Coat currentCoat = _serviceUser.nextItem(matching);
        while (true) {
            ...
            std::cout << "\nCurrent coat:\n";
            if (next) {
                currentCoat = _serviceUser.nextItem(matching);
                next = false;
            }
            ...
            if (option == "BUY") {
                ...
                amount = readInt();
                try {
                    _serviceUser.addToCart(currentCoat, amount, matching);
...
void ConsoleUser::runConsole() {
    ...
    while (true) {
        std::cout<<"Available commands!\n"
                   ...
                   "shopping basked - Display the content of the cart"
                   "exit - Exit the app\n";
