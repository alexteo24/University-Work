...
class Graph:
    ...
    def bellman(self, source, maximum_length):
        ...
        for k in range(1, maximum_length + 1):
            ...
            distance[k] = [e for e in previous_row]
...
graph = Graph()
read_graph_from_file(graph, "graph10k.txt")
start = 0
end = 4
maxLen = 6
minDist = 999999
...
for i in range(0, graph.number_vertices):
    if dist[-1][i] < dist[-2][i] and dist[-2][i] != 999999:
        print(dist[-1][i], dist[-2][i])
...
# Table, divided n*n, in fiecare celula are 1 marble sau nimic. You can remove a stone if there is another stone on the same
# line or column, until all the lines and columns have no stones left. Care e nr maxim de eliminari? Hint: Use graphs to find it