...
class Graph:
    ...
    def DFS(self, vertex, visited, string):
        visited[vertex] = 1
        string[0] += ' ' + str(vertex)
        for some_vertex in self.__dictionary_in[vertex]:
            if visited[some_vertex] == 0:
                self.DFS(some_vertex, visited, string)

    def get_connected_components(self):
        visited = [0 for i in range(0, self.__number_of_vertices + 1)]
        string = [' ']
        for index in range(0, self.__number_of_vertices + 1):
            if visited[index] == 0:
                self.DFS(index, visited, string)
                print(string[0])
                string[0] = ' '
